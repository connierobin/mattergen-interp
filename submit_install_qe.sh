...
apt install -y build-essential gfortran 2>&1
check_status "Build tools installation"

echo
echo "Installing MPI libraries..."
apt install -y libopenmpi-dev openmpi-bin 2>&1
check_status "MPI installation"

echo
echo "Installing scientific libraries..."
apt install -y libfftw3-dev liblapack-dev libblas-dev libscalapack-mpi-dev 2>&1
...
apt install -y quantum-espresso quantum-espresso-data 2>&1
check_status "Quantum ESPRESSO installation"
echo

echo "=== STEP 5: VERIFY INSTALLATION ==="
echo "Checking installed QE executables:"
ls -la /usr/bin/*pw* /usr/bin/*qe* 2>/dev/null || echo "No QE executables found"
...
ls -la /usr/share/espresso/pseudo/ 2>/dev/null | head -5 || echo "❌ Pseudopotentials not found"
echo "Total pseudopotentials: $(ls /usr/share/espresso/pseudo/ 2>/dev/null | wc -l)"
...
echo
echo "Running minimal QE test (should stop quickly on convergence or error):"
timeout 30s /usr/bin/pw.x < minimal_test.in 2>&1 | head -50 || echo "Test completed"

echo
echo "=== STEP 8: ENVIRONMENT SETUP ==="
echo "Setting up environment for ASE integration..."
echo "ASE_ESPRESSO_COMMAND should be: /usr/bin/pw.x < PREFIX.pwi > PREFIX.pwo"
...
echo
echo "Next steps:"
echo "1. Update ASE_ESPRESSO_COMMAND='/usr/bin/pw.x < PREFIX.pwi > PREFIX.pwo'"
echo "2. Test with actual DFT calculation"
echo "3. Integrate with mattergen DFT testing framework"
echo
echo "==============================================="